/**
 * SACERDOTI S.A. - Interacciones y Lógica de la Web
 */
...
function initLightbox() {
  ...
  const modalTitle = modal?.querySelector('.lightbox-caption h3');
  ...
  function updateModalImage(index) {
    ...
    const item = currentLightboxImages[activeLightboxIndex];
    if (modalImg) modalImg.src = item.src;
    if (modalTitle) modalTitle.textContent = item.title || 'Sacerdoti Gráfica';
    if (modalDesc) modalDesc.textContent = item.category || 'Muestra de Producto';
  }
  ...
  prevBtn?.addEventListener('click', () => updateModalImage(activeLightboxIndex - 1));
  nextBtn?.addEventListener('click', () => updateModalImage(activeLightboxIndex + 1));
  ...
  document.addEventListener('keydown', (e) => {
    ...
    if (e.key === 'ArrowLeft') updateModalImage(activeLightboxIndex - 1);
    if (e.key === 'ArrowRight') updateModalImage(activeLightboxIndex + 1);
  });

  // Attach click to all gallery items
  const galleryItems = document.querySelectorAll('.gallery-card');
  const imagesData = Array.from(galleryItems).map(card => {
    const img = card.querySelector('img');
    const title = card.querySelector('.gallery-card-title')?.textContent || '';
    const category = card.querySelector('.gallery-card-tag')?.textContent || '';
    return {
